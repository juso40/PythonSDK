...
LRESULT __stdcall WndProc(const HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	if (true && MEditor::bWantsEditorOpen && ImGui_ImplWin32_WndProcHandler(hWnd, uMsg, wParam, lParam)) {
		return true;
	}
...
namespace MEditor {
	...
	// At some point I will switch to a Class instead of abusing this namespace
	...
	std::vector<UObject*> vMaterialInstantConstants;
	std::vector<char*> vMaterialInstantConstantPathNames;
	...
	const char* cTexture2DWindowFor;
	...
	void RefreshMatInstConsts() {
		vMaterialInstantConstants = UObject::FindAll(const_cast<char*>("MaterialInstanceConstant"));
		vTexture2D = UObject::FindAll(const_cast<char*>("Texture2D"));

		vMaterialInstantConstantPathNames.clear();
		for (auto v : vMaterialInstantConstants) {
			vMaterialInstantConstantPathNames.push_back(UObject::PathName(v).AsString());
		}
	...
	UMaterial* GetParentMat() {
		...
		while (true) {
			...
			if (parent->IsA(UObject::FindClass("Material"))) {
				break;
			}
	...
	void GetTextureParameters() {
		...
		if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Materials.Weapons.Master_Gun") ||
			StrStrIA(parentMat->PathName(parentMat).AsString(), "Anemone_GunMaterials.MasterMaterials.Master_Gun_Anemone") ||
			StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Co_Materials.Weapons.Master_Gun") ||
			StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Ma_Materials.Weapon.Master_Gun_Glitched")) {

			const char* c[] = { "p_Decal", "p_Pattern", "P_SimpleReflect", "p_Masks", "p_NormalScopesEmissive", "p_Diffuse" };
			vTextureParameters.assign(std::begin(c), std::end(c));


		}
		else if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Materials.Items.Master_ShieldGrenade") ||
				 StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Materials.Items.Master_ClassMod_02")) {

			const char* c[] = { "p_Decal", "p_CustomPattern","P_SimpleReflect", "p_Masks", "p_EpicEmiss", "p_Diffuse" };
			vTextureParameters.assign(std::begin(c), std::end(c));
		}
		else {
			vTextureParameters.clear();
	...
	void GetVectorParameters() {
		...
		if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Materials.Weapons.Master_Gun") ||
			...
			StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Co_Materials.Weapons.Master_Gun") ||
			StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Ma_Materials.Weapon.Master_Gun_Glitched") ||
			StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Materials.Items.Master_ShieldGrenade")) {

			const char* c[] = { "p_AColorHilight", "p_AColorMidtone", "p_AColorShadow", "p_BColorHilight", "p_BColorMidtone", "p_BColorShadow", "p_CColorHilight", "p_CColorMidtone", "p_CColorShadow", "p_DColor",
			 "p_ReflectionChannelScale", "p_ReflectColor", "p_PatternChannelScale", "p_PatternScalePosition", "p_PatternColor", "p_DecalChannel", "p_DecalScalePosition", "p_DecalColor", "p_EmissiveColor", "p_DigiStructColor" };
			vVectorParameters.assign(std::begin(c), std::end(c));

			// only TPS
			if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Ma_Materials.Weapon.Master_Gun_Glitched")) {
				vVectorParameters.push_back("SpecialWeaponColor");
				vVectorParameters.push_back("GlitchColor");
				vVectorParameters.push_back("UV2TilingAndOffset");
			}
			if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Co_Materials.Weapons.Master_Gun")) {
				vVectorParameters.push_back("SpecialWeaponColor");
				vVectorParameters.push_back("p_MoonstoneOverlayScalePosition");
				vVectorParameters.push_back("p_MoonstoneColor_A");
				vVectorParameters.push_back("p_MoonstoneColor_B");
			}

		}
		else if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Materials.Items.Master_ClassMod_02")) {
			const char* c[] = { "p_AColorHilight", "p_AColorMidtone", "p_AColorShadow", "p_BColorHilight", "p_BColorMidtone", "p_BColorShadow", "p_CColorHilight", "p_CColorMidtone", "p_CColorShadow", "p_ColorD",
			 "p_ReflectionChannelScale", "p_ReflectColor", "p_PatternChannelScale", "p_PatternScalePosition", "p_PatternColor", "p_DecalChannel", "p_DecalScalePosition", "p_DecalColor", "p_EmissiveColor", "p_ShowPart" };
			vVectorParameters.assign(std::begin(c), std::end(c));
		}
		else {
			vVectorParameters.clear();
	...
	void GetScalarParameters() {
		...
		if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Materials.Weapons.Master_Gun") ||
			...
			const char* c[] = { "p_EmissiveScale", "p_HighlightsIntensity", "p_ShadowsIntensity", "p_ReflectColorScale", "p_ReplaceDecal", "p_UseFullColorDecal", "p_DecalRotate", "p_ReplacePattern", "p_DigiStruct" };
			vScalarParameters.assign(std::begin(c), std::end(c));
			// For TPS only
			if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Co_Materials.Weapons.Master_Gun")) {
				vScalarParameters.push_back("p_MoonstoneOverlay");
			}
			if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Ma_Materials.Weapon.Master_Gun_Glitched")) {
				vScalarParameters.push_back("p_GlitchOn");
				vScalarParameters.push_back("GlowContrastRangeMin");
				vScalarParameters.push_back("GlowContrastRangeMax");
			}

		}
		else if (StrStrIA(parentMat->PathName(parentMat).AsString(), "Common_Materials.Items.Master_ShieldGrenade")) {

			const char* c[] = { "p_EmissiveScale", "p_HighlightsIntensity", "p_ShadowsIntensity", "p_ReflectColorScale", "p_ReplaceDecal", "p_UseFullColorDecal", "p_DecalRotate", "p_PatternInvert", "p_EpicLegendaryEmissScale" };
			vScalarParameters.assign(std::begin(c), std::end(c));
		}
		else {
			vScalarParameters.clear();
	...
	HRESULT __stdcall Hooked_EndScene(IDirect3DDevice9* pDevice) // Our hooked endscene
	{
		...
		if (bWantsEditorOpen) {
			...
			if (ImGui::Button("Update MaterialInstantConstant List")) RefreshMatInstConsts();
			...
			if (ImGui::CollapsingHeader(Util::Format("MaterialInstanceConstant: %s", cSelectedMaterial).c_str())) {
				if (ImGui::InputText("Obj Filter", cFilterMats, IM_ARRAYSIZE(cFilterMats))) {
					FilterPathObjVecs(cFilterMats, &vMaterialInstantConstantPathNames, &vMaterialInstantConstants, &vMaterialsPathNameFiltered, &vMaterialsFiltered);
				}

				ImGui::Text(Util::Format("Selected: %s", cSelectedMaterial).c_str());
				if (ImGui::ListBoxHeader("MaterialInstantConstant")) {
			...
			if (ImGui::CollapsingHeader(Util::Format("Parent: %s", cSelectedMaterialParent).c_str())) {
				if (SelectedMaterial == nullptr) {
					...
					if (ImGui::InputText("Obj Filter", cFilterMatsParents, IM_ARRAYSIZE(cFilterMatsParents))) {
						FilterPathObjVecs(cFilterMatsParents, &vMaterialInstantConstantPathNames, &vMaterialInstantConstants, &vMaterialsPathNameFiltered, &vMaterialsFiltered);
					}
